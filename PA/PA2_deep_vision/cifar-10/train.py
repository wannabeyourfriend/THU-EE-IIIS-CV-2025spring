...
def MyCELoss(pred, gt):
    ...
    gt_one_hot.scatter_(1, gt.unsqueeze(1), 1)
    ...
    epsilon = 1e-10
    pred_softmax = torch.clamp(pred_softmax, epsilon, 1.0 - epsilon)
    
    # Calculate cross-entropy loss
    loss = -torch.sum(gt_one_hot * torch.log(pred_softmax)) / batch_size
    return loss 
...
def validate(epoch, model, val_loader, writer):
    ...
    # draw accuracy curve!
    # ----------TODO------------
    # 使用TensorBoard记录验证准确率
    writer.add_scalar('Validation/Acc@1', top1.avg, epoch)
    writer.add_scalar('Validation/Acc@5', top5.avg, epoch)
...
def train(epoch, model, optimizer, criterion, train_loader, writer):
    ...
    for imgs, labels in tqdm.tqdm(train_loader):
        ...
        if iteration % 50 == 0:
            ...
            writer.add_scalar('Train/Acc@1', top1.val, iteration)
            writer.add_scalar('Train/Acc@5', top5.val, iteration)
